...
At the heart of dep is a constraint solving engine - a [CDCL]()-style [SMT]() solver, tailored specifically to the domain of Go package management. It lives in the `github.com/asticode/dep/gps` package, and is where the work of determining a valid, transitively complete dependency graph (aka, the contents of `Gopkg.lock`) is performed.
...
> Note: dep does not currently enforce this rule, but [it needs to](https://github.com/asticode/dep/issues/902).

...
Keep an eye on the [Roadmap](https://github.com/asticode/dep/wiki/Roadmap) for a summary of where the project is, and where we're headed.

## Filing issues

Please check the existing issues and [FAQ](docs/FAQ.md) to see if your feedback has already been reported.

When [filing an issue](https://github.com/asticode/dep/issues/new), make sure to answer these five questions:
...
The
[help wanted](https://github.com/asticode/dep/issues?q=is%3Aissue+is%3Aopen+label%3A%22help%20wanted%22)
label highlights issues that are well-suited for folks to jump in on. The
[good first issue](https://github.com/asticode/dep/issues?q=is%3Aissue+is%3Aopen+label%3A%22good%20first%20issue%22)
...
* _Be kind, respectful, and inclusive_. Really live that [CoC](https://github.com/asticode/dep/blob/master/CODE_OF_CONDUCT.md). We've developed a reputation as one of the most welcoming and supportive project environments in the Go community, and we want to keep that up!
...
* Remember, the long-term goal of `dep` is to disappear into the `go` toolchain. That's going to be a challenging process, no matter what. Minimizing that eventual difficulty should be a guiding light for all your decisions today.
  * Try to match the toolchain's assumptions as closely as possible ([example](https://github.com/asticode/dep/issues/564#issuecomment-300994599)), and avoid introducing new rules the toolchain would later have to incorporate.
...
* We use [Zenhub](https://www.zenhub.com) to manage the queue, in addition to what we do with labels.
  ...
  * Pipelines, and [the board](https://github.com/asticode/dep#boards) are one thing we try to utilize:
...
* The [`help wanted`](https://github.com/asticode/dep/issues?q=is%3Aissue+is%3Aopen+label%3A%22help+wanted%22) and [`good first issue`](https://github.com/asticode/dep/labels/good%20first%20issue) labels are two of our most important tools for making the project accessible to newcomers - a key goal for our community. Here's how to use them well.
